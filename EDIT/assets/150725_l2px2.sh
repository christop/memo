...
# --------------------------------------------------------------------------- #
# layers2pagesx2  (transform layer to doublepages)                            #
# -     --    --                                                              #
#                                                                             #
#  Copyright (C) 2014 LAFKON/Christoph Haag                                   #
#                                                                             #
#  150725_l2px2.sh is free software: you can redistribute it and/or modify    # 
#  it under the terms of the GNU General Public License as published by the   # 
#  Free Software Foundation, either version 3, or (at your option)            #
#  any later version.                                                         #
#                                                                             #
#  150725_l2px2.sh is distributed in the hope that it will be useful          #
...
# --------------------------------------------------------------------------- #
  ...
  COUNT=1
  for LAYERTYPE in `sed -n '1!p' ${SVG%%.*}.tmp        | # DISPLAY EVERYTHING EXCEPT FIRST LINE
   ...
   do
      ...
      do
          ...
          grep ":label=\"$LAYERTYPE" ${SVG%%.*}.tmp | # THE LAYER
          sed "s/$BREAKFOO/\n/g" | # RESTORE ORIGINAL LINEBREAKS
          sed "s/$SPACEFOO/ /g"  | # RESTORE ORIGINAL SPACES
          tee                    >>  ${OUT}.svg
          echo "</g>"            >>  ${OUT}.svg
